...
/* =========================================================
   ...
   - + KYC 等级(kvcTier) 与过期时间(kycExpireAt)
   - + EIP-712 后台签名自动上白 setWhitelistBySig(...)
...
contract ComplianceGuardV2 is Ownable, EIP712 {
    ...
    constructor(address owner_, address verifier_) Ownable(owner_) EIP712("ComplianceGuardV2", "1") {
        verifier = verifier_;
