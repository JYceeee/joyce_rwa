...
export default {
  ...
  methods: { 
    ...
    viewProjectDetails(projectCode) {
      // 跳转到项目详情页面
      console.log('🏠 HomeView: 尝试跳转到项目详情页面:', projectCode)
      this.$router.push(`/detail/${projectCode}`).then(() => {
        console.log('✅ HomeView: 成功跳转到项目详情页面')
      }).catch((error) => {
        console.error('❌ HomeView: 跳转失败:', error)
      })
    },
    ...
    viewAllProjects() {
      // 跳转到项目列表页面
      console.log('🏠 HomeView: 尝试跳转到项目列表页面...')
      this.$router.push('/listedprojects').then(() => {
        console.log('✅ HomeView: 成功跳转到项目列表页面')
      }).catch((error) => {
        console.error('❌ HomeView: 跳转失败:', error)
      })
    },
    ...
    // 跳转到项目页面
    goToProjects() {
      console.log('🏠 HomeView: 尝试跳转到项目页面...')
      this.$router.push('/listedprojects').then(() => {
        console.log('✅ HomeView: 成功跳转到项目页面')
      }).catch((error) => {
        console.error('❌ HomeView: 跳转失败:', error)
      })
    },
  ...
  async mounted() {
    ...
    // 清除任何现有的视频播放记录，确保每次都会播放
    localStorage.removeItem('rwa_intro_seen')
    
    // 每次进入都自动播放开场视频
    console.log('🎬 HomeView: 自动播放开场视频')
    this.showIntroVideo = true
    this.introVideoLoading = true
    this.introVideoError = false
    this.showIntroSkipButton = false
